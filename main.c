...
typedef struct {
    pid_t pid;      /* cleared to -1 once the process has finished */
    int   first_run;
    int   finish;
    int   quanta;
} pcb;
...
static void worker_run(int idx)
{
    for (int r = 1; r <= JOBS[idx].rounds; r++) {
        burn(JOBS[idx].burst);
        printf("    [P%d %-3s] round %d of %d\n",
               idx + 1, JOBS[idx].name, r, JOBS[idx].rounds);
        if (JOBS[idx].wait_ms)
...
static void sched_record(sim *s)
{
    for (int i = 0; i < NPROCS; i++)
        s->chart[i][s->tick] = s->procs[i].pid < 0 ? ' '
                             : i == s->running     ? '#'
                             :                       '.';
}
...
/* The handler does nothing but raise a flag. Anything else, printf above all,
